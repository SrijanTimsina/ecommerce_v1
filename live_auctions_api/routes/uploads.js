...
router.post('/image', isAuth, upload.single('image'), async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ imagePath:`${result.public_id}`});
  } catch (error) {
    console.log(error);
    res.status(500).json({ errors: { msg: 'Server error in upload' } });
  }
});
...
router.get('/image/:key', async (req, res) => {
  try {
    ...
    console.log(error);
    res.status(500).json({ errors: { msg: 'Server error in upload2' } });
  }
});
