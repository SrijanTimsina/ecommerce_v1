...
exports.registerUser = async (req, res, next) => {
  try {
    ...
    console.log(err);
    res.status(500).json({ errors: [{ msg: 'User already created' }] });
  }
};
...
exports.purchasedProducts = async (req, res) => {
  ...
  try {
    ...
    console.log(error);
    res.status(500).json({ errors: [{ msg: 'Server error in user' }] });
  }
};
...
exports.postedProducts = async (req, res) => {
  ...
  try {
    ...
    console.log(error);
    res.status(500).json({ errors: [{ msg: 'Server error in users' }] });
  }
};
