...
exports.addAd = async (req, res, next) => {
	...
	try {
		...
		res
			.status(500)
			.json({ errors: [{ msg: "Server error in adjs" }] });
	}
};
...
exports.retrieveAds = async (req, res, next) => {
	...
	try {
		...
		console.log(err);
		res.status(500).json({ errors: [{ msg: "Server error  here" }] });
	}
};
	...
	try {
		...
		if (ad.owner != req.user.id)
			return res.status(401).json({
				errors: [{ msg: "Unauthorized to delete this ad" }],
			});
		...
		res
			.status(500)
			.json({ errors: [{ msg: "Server error in here" }] });
	}
	// console.log(adId);
};
	...
	try {
		...
		if (ad.auctionStarted || ad.auctionEnded)
			return res.status(404).json({
				errors: [{ msg: "Cannot delete, auction started/ended" }],
			});
		...
		res
			.status(500)
			.json({ errors: [{ msg: "Server error in  addel" }] });
	}
};
